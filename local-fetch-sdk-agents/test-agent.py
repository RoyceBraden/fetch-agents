...
def init_client():
    ...
    try:
        ...
        readme = """
        <description>Agent for testing the text moderation agent</description>
        <use_cases>
            <use_case>Test whether text moderation agent works</use_case>
        </use_cases>
        """

        register_with_agentverse(
            identity=client_identity,
            url="http://localhost:1111/api/webhook",
...
def send_data():
    ...
    try:
        ...
        MODERATION_AGENT_ADDRESS = "agent1qv5j8kw7shanr802r274y46zsgssxasljkjg465fr6yz2kmcf4vpx5ut63r"

        if not prompt:
            return jsonify({"error": "Missing prompt"}), 400

        logger.info(f"Sending prompt {prompt} to {MODERATION_AGENT_ADDRESS}")

        payload = {"prompt": prompt} 
        send_message_to_agent(
            client_identity,
            MODERATION_AGENT_ADDRESS,
            payload
        )
